...
export class NarrationRelayServer {
  ...
  async start(): Promise<void> {
    ...
    this.wss.on("error", (err) => logger.error("Narration WebSocket error:", err));

    return new Promise<void>((resolve, reject) => {
      this.server!.on("error", (err: NodeJS.ErrnoException) => {
        if (err.code === "EADDRINUSE") {
          logger.error(`Narration relay port ${this.port} is already in use.`);
        }
        reject(err);
      });
      this.server!.listen(this.port, () => {
        logger.info(`Narration relay listening on http://localhost:${this.port}`);
        logger.info(`Narration WebSocket endpoint: ws://localhost:${this.port}/ws/narration`);
        resolve();
      });
    });
  }
  ...
  async stop(): Promise<void> {
    ...
    if (this.server) {
      return new Promise<void>((resolve) => {
        this.server!.close(() => {
          logger.info("Narration relay stopped");
          this.server = null;
          resolve();
        });
      });
  ...
  getUiClientCount(): number {
    return [...this.clients].filter((client) => client.role === "ui").length;
  }
  ...
  publishSay(input: Omit<NarrationSayMessage, "type" | "id"> & { id?: string }): string {
    ...
    if (this.getUiClientCount() === 0) {
      logger.debug(`[Narration] skipped ${id}: no UI clients`);
      this.broadcastStatus({ type: "narration:skipped", id, timestamp: Date.now() });
      return id;
    }

    const timer = setTimeout(() => {
      logger.warn(`[Narration] timed out waiting for completion: ${id}`);
      this.finishPending({
  ...
  private handleConnection(ws: WebSocket): void {
    ...
    ws.on("message", (raw) => {
      try {
        ...
        logger.error("Invalid narration WebSocket message:", err);
      }
    });
    ...
    ws.on("error", (err) => {
      logger.error("Narration WebSocket client error:", err);
      this.clients.delete(client);
      this.broadcastState();
    });
  }
  ...
  private handleMessage(client: Client, msg: NarrationClientMessage): void {
    switch (msg.type) {
      case "narration:hello":
        ...
        logger.info(`[Narration] client role=${client.role}${client.clientName ? ` name=${client.clientName}` : ""}`);
        this.send(client, {
          type: "narration:ready",
          role: client.role,
          uiClients: this.getUiClientCount(),
